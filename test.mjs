...
function assertClose (actual, expected, tolerance, message) {
  ...
  if (diff <= tolerance) {
    ...
    console.error(
      `✗ ${message} - Expected ${expected.toFixed(4)}, got ${actual.toFixed(4)}`
    )
    failed++
  }
}
...
assert(compareStrings('hello', 'hello') === 1, 'Identical strings return 1.0')

assert(compareStrings('', '') === 0, 'Empty strings return 0')

assert(compareStrings('hello', '') === 0, 'One empty string returns 0')

assert(compareStrings(null, 'test') === 0, 'Null value returns 0')
...
assert(compareStrings('  hello  ', 'hello') === 1, 'Whitespace trimming works')
...
assertEquals(stripDiacritics('café'), 'cafe', 'Removes accent from café')

assertEquals(stripDiacritics('áéíóú'), 'aeiou', 'Removes accents from áéíóú')

assertEquals(stripDiacritics('Zürich'), 'Zurich', 'Removes umlaut from Zürich')

assertEquals(stripDiacritics('hello'), 'hello', 'Leaves plain ASCII unchanged')

assertEquals(stripDiacritics('naïve'), 'naive', 'Removes diaeresis from naïve')
...
assertEquals(stripDiacritics('façade'), 'facade', 'Removes cedilla from façade')
...
assertEquals(sanitizeName('Dr. Jane Doe'), 'jane doe', 'Removes Dr. honorific')
...
assertEquals(sanitizeName("O'Brien"), 'o brien', 'Normalizes apostrophe')

assertEquals(sanitizeName(null), '', 'Handles null input')

assertEquals(sanitizeName(''), '', 'Handles empty string')

assertEquals(sanitizeName('Prof. Smith'), 'smith', 'Removes Prof. honorific')
...
assertEquals(structuralBonus('', 'john'), 0, 'Empty string returns 0')
...
assert(
  compareNames('John Smith', 'Jane Smith') > 0.5 &&
    compareNames('John Smith', 'Jane Smith') < 0.8,
...
assert(compareNames('', '') === 0, 'Empty names return 0')

assert(compareNames(null, 'John') === 0, 'Null name returns 0')
...
assert(compareStrings('a', 'a') === 1, 'Single character exact match')

assert(compareStrings('a', 'b') < 0.5, 'Single character mismatch')

assertEquals(sanitizeName('123'), '123', 'Numeric string passes through')
